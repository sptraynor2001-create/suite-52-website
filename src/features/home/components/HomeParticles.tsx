...
// Main particles
const PARTICLE_COUNT = 800 // Increased for more density on outside
...
export function WhirlpoolParticles({ mouseX = 0, mouseY = 0 }: WhirlpoolParticlesProps) {
  ...
  const prevPositions = useRef<Float32Array>(new Float32Array(PARTICLE_COUNT * 3))
  const glitchStates = useRef<Float32Array>(new Float32Array(PARTICLE_COUNT))
  const trailAges = useRef<Float32Array>(new Float32Array(TRAIL_COUNT)) // Age of each trail particle
  const fadeTimers = useRef<Float32Array>(new Float32Array(PARTICLE_COUNT)) // Fade timer for each particle (0 = not fading, >0 = fading)
  const FADE_DURATION = 2.5 // Fade out over 2.5 seconds

  // Particle data with randomness
  const particles = useMemo(() => {
    const positions = new Float32Array(PARTICLE_COUNT * 3)
    const colors = new Float32Array(PARTICLE_COUNT * 3) // RGB colors for opacity control
    ...
    const canGlitch = new Float32Array(PARTICLE_COUNT)
    const sizes = new Float32Array(PARTICLE_COUNT) // Size variation per particle
    const glitchRates = new Float32Array(PARTICLE_COUNT) // Individual glitch rates
    const glitchOffsets = new Float32Array(PARTICLE_COUNT) // Individual glitch offset amounts
  ...
  const trailParticles = useMemo(() => {
    ...
    const sourceIndices = new Uint16Array(TRAIL_COUNT)
    
    for (let i = 0; i < TRAIL_COUNT; i++) {
      // Each trail follows a random main particle
      sourceIndices[i] = Math.floor(Math.random() * PARTICLE_COUNT)
    ...
    for (let i = 0; i < PARTICLE_COUNT; i++) {
      ...
      if (fadeTimers.current[i] > 0) {
        ...
        if (fadeTimers.current[i] >= FADE_DURATION) {
          ...
          const opacityMultiplier = isWeb ? 0.25 + (mouseY + 1) * 0.375 : 1.0
      ...
      // Mouse interaction (web only): X controls spiral speed, Y controls opacity (25-100%)
      const isWeb = typeof window !== 'undefined' && window.innerWidth >= 768
      const speedMultiplier = isWeb ? 1 + mouseX * 0.01 : 1 // Left = slightly slower (0.99x), Right = slightly faster (1.01x) - very subtle
      // Map mouseY (-1 to 1) to opacity (0.25 to 1.0)
      const opacityMultiplier = isWeb ? 0.25 + (mouseY + 1) * 0.375 : 1.0 // Down = 25% opacity, Up = 100% opacity
      
      // Calculate progress - apply speed multiplier to time for spiral velocity
      const cycleLength = 120
      const adjustedTime = ((time * speedMultiplier) + lifetime) % cycleLength
      ...
      if (currentRadius < 0.3 && fadeTimers.current[i] === 0) {
        fadeTimers.current[i] = 0.001 // Start fade timer
      }
      
      const rotationMultiplier = 1 + (1 - currentRadius / startRadius) * 1.2 // Less aggressive acceleration
      const currentAngle = particles.angles[i] + time * rotationSpeed * rotationMultiplier
...
const ACCENT_COUNT = 130 // Increased from 80 (~18% of total particles)
const ACCENT_TRAIL_COUNT = 150
const ACCENT_TRAILS_PER_GLITCH = 5

export function AccentParticles({ mouseX = 0, mouseY = 0 }: WhirlpoolParticlesProps) {
  const pointsRef = useRef<THREE.Points>(null)
  const trailRef = useRef<THREE.Points>(null)
  const prevPositions = useRef<Float32Array>(new Float32Array(ACCENT_COUNT * 3))
  const glitchStates = useRef<Float32Array>(new Float32Array(ACCENT_COUNT))
  const trailAges = useRef<Float32Array>(new Float32Array(ACCENT_TRAIL_COUNT))
  const fadeTimers = useRef<Float32Array>(new Float32Array(ACCENT_COUNT))
  const ACCENT_FADE_DURATION = 2.5

  const particles = useMemo(() => {
    const positions = new Float32Array(ACCENT_COUNT * 3)
    const colors = new Float32Array(ACCENT_COUNT * 3) // RGB colors for opacity control
    const angles = new Float32Array(ACCENT_COUNT)
    const startRadii = new Float32Array(ACCENT_COUNT)
    const rotationSpeeds = new Float32Array(ACCENT_COUNT)
    const lifetimes = new Float32Array(ACCENT_COUNT)
    const zOffsets = new Float32Array(ACCENT_COUNT)
    const canGlitch = new Float32Array(ACCENT_COUNT)
    const sizes = new Float32Array(ACCENT_COUNT)
    const glitchRates = new Float32Array(ACCENT_COUNT)
    const glitchOffsets = new Float32Array(ACCENT_COUNT)
    ...
    for (let i = 0; i < ACCENT_COUNT; i++) {
      ...
      // Size variation
      sizes[i] = 0.045 + Math.random() * 0.015 // 0.045 to 0.06
  ...
  const trailParticles = useMemo(() => {
    const positions = new Float32Array(ACCENT_TRAIL_COUNT * 3)
    const sourceIndices = new Uint16Array(ACCENT_TRAIL_COUNT)
    
    for (let i = 0; i < ACCENT_TRAIL_COUNT; i++) {
      sourceIndices[i] = Math.floor(Math.random() * ACCENT_COUNT)
      positions[i * 3] = 1000
      positions[i * 3 + 1] = 1000
      positions[i * 3 + 2] = 1000
    }
    ...
    for (let i = 0; i < ACCENT_COUNT; i++) {
      ...
      if (fadeTimers.current[i] > 0) {
        ...
        if (fadeTimers.current[i] >= ACCENT_FADE_DURATION) {
          ...
          particles.sizes[i] = 0.045 + Math.random() * 0.015
      ...
      // Start fade when particle reaches center
      if (currentRadius < 0.4 && fadeTimers.current[i] === 0) {
        fadeTimers.current[i] = 0.001 // Start fade timer
      }
      
      const rotationMultiplier = 1 + (1 - currentRadius / startRadius) * 1.0 // Less aggressive acceleration
      const currentAngle = particles.angles[i] + time * rotationSpeed * rotationMultiplier
